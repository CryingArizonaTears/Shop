...
ALTER TABLE product ADD FOREIGN KEY (warehouse_id)
...
ALTER TABLE user_profile ADD FOREIGN KEY (user_credentials_id)
    REFERENCES user_credentials (id)
    ON DELETE CASCADE;
    
ALTER TABLE user_profile ADD FOREIGN KEY (role_id)
    REFERENCES role (id)
    ON DELETE CASCADE;
    
ALTER TABLE user_profile ADD FOREIGN KEY (bucket_id)
...
ALTER TABLE order_ ADD FOREIGN KEY (user_id)
    REFERENCES user_profile (id)
    ON DELETE CASCADE;

ALTER TABLE order_ ADD FOREIGN KEY (currency_id)
...
ALTER TABLE order_processed ADD FOREIGN KEY (order_id )
    REFERENCES order_ (id)
    ON DELETE CASCADE;
    
DELIMITER 
CREATE TRIGGER order_to_order_processed 
AFTER
UPDATE ON order_
FOR EACH ROW
BEGIN
	IF NEW.processed = true THEN
		INSERT INTO order_processed
		SET order_id = NEW.id, processed_date = NOW();
	END IF;
END;
...
DESCRIBE role;
DESCRIBE bucket;
DESCRIBE bucket_product;
DESCRIBE order_;
DESCRIBE product_order;
DESCRIBE order_processed;
